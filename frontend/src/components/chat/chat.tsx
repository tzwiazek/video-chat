...
import { ChatContainer, TopHeader, ChatArea, MessageList, Message, UserMessage, BottomInput } from './chat.styles';
...
const Chat = ({ display, roomID }: { display: boolean, roomID: string }) => {
  ...
  useEffect(() => {
    socket.on('CHAT-receive-message', ({ message, sender }: { message: string, sender: string }) => {
  ...
  return (
    <ChatContainer className={display ? '' : 'hide-chat'}>
      ...
      <ChatArea>
        <MessageList>
          {message &&
            message.map(({ sender, message }: { sender: string, message: string }, index: number) => {
